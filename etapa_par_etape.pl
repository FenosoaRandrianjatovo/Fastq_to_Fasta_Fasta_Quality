...
open(my $fasqual_out, ">", "qual.txt") or die "Impossible d'ouvrir le fichier de sortie : $!";
...
while (my $line = <$fastq_in>) {
    ...
    if ($line =~ /^@/) {
        # Remplace le "@" par ">" et écrit l'identifiant dans le fichier de sortie
        print $fasqual_out ">", substr($line, 1), "\n";
        ...
        # Écrit les scores de qualité convertis dans le fichier de sortie
        print $fasqual_out join(" ", @quality_scores), "\n";
    }
}
...
print "Conversion au format FASQUAL terminée avec succès.\n";